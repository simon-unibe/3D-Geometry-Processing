...
bool is_delaunay(TriMesh const &_mesh, TriMesh::EdgeHandle _eh) {
  ...
  ACG::Matrix4x4d circle_test;
  circle_test(0, 0) = a[0];
  circle_test(0, 1) = a[1];
  circle_test(0, 2) = a[0] * a[0] + a[1] * a[1];
  circle_test(0, 3) = 1.0;
  circle_test(1, 0) = b[0];
  circle_test(1, 1) = b[1];
  circle_test(1, 2) = b[0] * b[0] + b[1] * b[1];
  circle_test(1, 3) = 1.0;
  circle_test(2, 0) = c[0];
  circle_test(2, 1) = c[1];
  circle_test(2, 2) = c[0] * c[0] + c[1] * c[1];
  circle_test(2, 3) = 1.0;
  circle_test(3, 0) = d[0];
  circle_test(3, 1) = d[1];
  circle_test(3, 2) = d[0] * d[0] + d[1] * d[1];
  circle_test(3, 3) = 1.0;

  return circle_test.determinant() <= 0.0;
}

/// Insert point into planar (z=0) delaunay mesh and re-establish delaunay
/// praoperty. Return the number of flips that were performed.
size_t insert_point(TriMesh &_mesh, const TriMesh::FaceHandle &_fh,
  ...
  //      after flipping, add edges to the queue whose incident triangles were
  //      changed by the flip to re-check them.

  for (auto iter = _mesh.cvoh_begin(vh); iter != _mesh.cvoh_end(vh); iter++) {
    edges_to_check.push(iter->next().edge());
  }

  while (!edges_to_check.empty()) {
    auto eh = edges_to_check.front();
    edges_to_check.pop();
    if (!is_delaunay(_mesh, eh) && _mesh.is_flip_ok(eh)) {
      _mesh.flip(eh);
      _mesh.make_smart(eh).h0().next().edge();
      _mesh.make_smart(eh).h0().prev().edge();
      _mesh.make_smart(eh).h1().next().edge();
      _mesh.make_smart(eh).h1().prev().edge();
    }
  }
...
public:
  DelaunayToolbar(QWidget *parent = nullptr) : QWidget(parent) {
    setupUi(this);
  }
};
...
void DelaunayPlugin::initializePlugin() {
  ...
  connect(tool_->create_test_mesh, &QPushButton::clicked, this,
          &DelaunayPlugin::slot_create_initial_mesh);
  connect(tool_->pb_reset_mesh, &QPushButton::clicked, this,
          &DelaunayPlugin::slot_reset_mesh);
  connect(tool_->pb_set_2d_view, &QPushButton::clicked, this,
          &DelaunayPlugin::slot_set_2DView);
...
void DelaunayPlugin::slotMouseEvent(QMouseEvent *_event) {
  ...
  if (PluginFunctions::actionMode() != Viewer::PickingMode)
    return;
  if (PluginFunctions::pickMode() != (pickmode_name))
    return;
  ...
  if (PluginFunctions::scenegraphPick(ACG::SceneGraph::PICK_EDGE, _event->pos(),
    ...
    if (!tri_obj->mesh()->is_valid_handle(eh))
      return;

    emit log(LOGINFO, "Adding point on edge");
    // insert the respective point into the triangulation
    add_cone(tri_obj, hit_point);
    auto n_flips = insert_point(*tri_obj->mesh(), TriMesh::InvalidFaceHandle,
                                eh, hit_point);
    emit log(LOGINFO, "Restored Delaunay property using " +
                          QString::number(n_flips) + " edge flips.");
  ...
  if (PluginFunctions::scenegraphPick(ACG::SceneGraph::PICK_FACE, _event->pos(),
    ...
    if (!tri_obj->mesh()->is_valid_handle(fh))
      return;
    // insert the respective point into the triangulation
    emit log(LOGINFO, "Adding point in face");
    add_cone(tri_obj, hit_point);
    auto n_flips = insert_point(*tri_obj->mesh(), fh,
                                TriMesh::InvalidEdgeHandle, hit_point);
    emit log(LOGINFO, "Restored Delaunay property using " +
                          QString::number(n_flips) + " edge flips.");
    emit updatedObject(tri_obj->id(), UPDATE_ALL);
    return;
...
TriMeshObject &DelaunayPlugin::getOrCreateMeshObject() {
  if (auto *tmo = PluginFunctions::triMeshObject(mesh_obj_id_)) {
    ...
    std::array<TriMesh::VertexHandle, 4> vhs = {
        mesh->add_vertex({-.5, -.5, 0.}), mesh->add_vertex({.5, -.5, 0.}),
        mesh->add_vertex({.5, .5, 0.}), mesh->add_vertex({-.5, .5, 0.})};
...
void DelaunayPlugin::add_cone(TriMeshObject *_tri_obj,
                              ACG::Vec3d const &_point) {
  // add cones
  auto node = getOrCreateGlutPrimitiveNode(_tri_obj);
  auto axis = ACG::Vec3d(0, 0, 1);
  // add an offset to the cones position, since the position is not located at
  // the apex but at the intersection of axis and base plate
  auto offset = ACG::Vec3d(0, 0, -1);

  node->add_primitive(ACG::SceneGraph::GlutPrimitiveNode::CONE, _point + offset,
                      axis, hcolors_.get_next_color());
}
...
DelaunayPlugin::getOrCreateGlutPrimitiveNode(TriMeshObject *_tri_obj) {
  ...
  if (!_tri_obj->hasAdditionalNode(name(), "PrimitiveMaterialNode")) {
    ...
    if (!_tri_obj->addAdditionalNode(material_node, QString(name()),
                                     "PrimitiveMaterialNode")) {
      emit(LOGERR, "DelaunayPlugin::getOrCreateGlutPrimitiveNode(): could not "
                   "add an primitive material node");
      return nullptr;
    }
  } else {
  ...
  if (!_tri_obj->hasAdditionalNode(name(), "GlutPrimitiveNode")) {
    glut_node = new ACG::SceneGraph::GlutPrimitiveNode(material_node,
                                                       "Voronoi diagram cones");
    if (!_tri_obj->addAdditionalNode(glut_node, name(), "GlutPrimitiveNode")) {
      emit(LOGERR, "DelaunayPlugin::getOrCreateGlutPrimitiveNode(): could not "
                   "add glut primitive node\n");
      return nullptr;
    }
    glut_node->clear();
